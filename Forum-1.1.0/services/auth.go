...
func GoogleCallback(c *gin.Context) {
	...
	if err != nil {
		if err.Error() == fmt.Sprintf("no user with email %s", googleUser.Email) {
			...
			_,err := db_manager.InsertUser(googleUser.Name, googleUser.Email, "", user_token)
			if err != nil {
				c.Redirect(http.StatusTemporaryRedirect, "/")
				return
			}
...
func GithubCallback(c *gin.Context) {
	...
	if err != nil {
		if err.Error() == fmt.Sprintf("no user with email %s", githubUser.Email) {
			...
			_,err := db_manager.InsertUser(githubUser.Login, githubUser.Email, "", user_token)
			if err != nil {
				c.Redirect(http.StatusTemporaryRedirect, "/")
				return
			}
...
func FacebookCallback(c *gin.Context) {
	...
	if err != nil {
		if err.Error() == fmt.Sprintf("no user with email %s", fbUser.Email) {
			...
			_,err := db_manager.InsertUser(fbUser.Name, fbUser.Email, "", user_token)
			if err != nil {
				c.Redirect(http.StatusTemporaryRedirect, "/")
				return
			}
